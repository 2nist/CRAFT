...
function Header({ onNewQuote }) {
  return (
    <div className="flex justify-between items-center mb-6">
      ...
      <button onClick={onNewQuote} className="inline-flex items-center gap-2 px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700">
      ...
      <div className="p-4 flex justify-between items-center border-b border-gray-700">
        <div className="relative w-full max-w-xs">
          <input
            ...
            className="w-full bg-gray-700 border border-gray-600 rounded-md p-2 pl-10 text-white"
          />
          <Search className="w-[18px] h-[18px] absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" />
        </div>
        <button onClick={fetchDeals} disabled={isLoading} className="ml-4 px-4 py-2 bg-gray-700 text-white rounded-md hover:bg-gray-600">
      ...
      <div className="overflow-x-auto">
        <table className="w-full">
          <thead className="bg-gray-900">
            <tr>
              <th className="px-4 py-3 text-left text-sm font-semibold text-gray-300">Deal Title</th>
              <th className="px-4 py-3 text-left text-sm font-semibold text-gray-300">Organization</th>
          ...
          <tbody>
            ...
            {!isLoading && filteredDeals.map(deal => (
              <tr key={deal.id} className="border-b border-gray-700 hover:bg-gray-700">
                <td className="px-4 py-3 font-medium text-white">{deal.title}</td>
                <td className="px-4 py-3 text-gray-300">{deal.org_name}</td>
                <td className="px-4 py-3 text-gray-300">{deal.owner_name}</td>
                <td className="px-4 py-3">
                  <span className="px-2 py-1 text-xs font-medium rounded-full bg-blue-900 text-blue-300">{deal.stage}</span>
                </td>
                <td className="px-4 py-3 text-gray-300">${deal.value.toLocaleString()}</td>
                <td className="px-4 py-3">
                  <button onClick={() => handleSelectDeal(deal)} className="px-3 py-1 bg-blue-600 text-white text-sm rounded-md hover:bg-blue-700">
                    Create Quote
                  </button>
                </td>
              </tr>
    ...
    <div className="bg-gray-800 rounded-lg shadow-lg overflow-hidden">
      ...
      <div className="overflow-x-auto">
        <table className="w-full">
          <thead className="bg-gray-900">
            <tr>
              <th className="px-4 py-3 text-left text-sm font-semibold text-gray-300">Project Name</th>
              <th className="px-4 py-3 text-left text-sm font-semibold text-gray-300">Quote ID</th>
              <th className="px-4 py-3 text-left text-sm font-semibold text-gray-300">Customer</th>
              <th className="px-4 py-3 text-left text-sm font-semibold text-gray-300">Status</th>
              <th className="px-4 py-3 text-left text-sm font-semibold text-gray-300">Last Modified</th>
              <th className="px-4 py-3"></th>
            </tr>
          </thead>
          <tbody>
            ...
            {!isLoading && filteredQuotes.length === 0 && (
              <tr><td colSpan="6" className="text-center p-6 text-gray-400">No existing quotes found.</td></tr>
            )}
            {!isLoading && filteredQuotes.map(quote => (
              <tr key={quote.quoteId} className="border-b border-gray-700 hover:bg-gray-700">
                ...
                <td className="px-4 py-3 text-gray-400 font-mono text-xs">{quote.quoteId}</td>
                ...
                <td className="px-4 py-3 text-gray-300 text-sm">{new Date(quote.lastModified).toLocaleString()}</td>
                <td className="px-4 py-3 text-right">
                  <div className="flex gap-2">
                    <button onClick={() => handleGenerateDocument(quote)} className="px-3 py-1 bg-gray-700 text-white text-sm rounded-md hover:bg-gray-600">
                      Generate Doc
                    </button>
                    <button onClick={() => handleEditQuote(quote)} className="px-3 py-1 bg-blue-600 text-white text-sm rounded-md hover:bg-blue-700">
                      Edit
                    </button>
